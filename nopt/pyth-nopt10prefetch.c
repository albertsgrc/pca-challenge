...
void compute_triples_from(unsigned int x, unsigned int y, unsigned int z) {
    ...
    if (__builtin_expect(zp3 <= N, 0)) {
        __builtin_prefetch(&mem[zp3], 0, 0);
        __builtin_prefetch(&mem[zp3], 1, 0);
        compute_triples_from(x + 2*y + 2*z, 2*x + y + 2*z, zp3);
    }
    if (__builtin_expect(zp1 <= N, 0)) {
        __builtin_prefetch(&mem[zp1], 0, 0);
        __builtin_prefetch(&mem[zp1], 1, 0);
        compute_triples_from(x - 2*y + 2*z, 2*x - y + 2*z, zp1);
    }
    if (__builtin_expect(zp2 <= N, 0)) {
        __builtin_prefetch(&mem[zp2], 0, 0);
        __builtin_prefetch(&mem[zp2], 1, 0);
        compute_triples_from(-x + 2*y + 2*z, -2*x + y + 2*z, zp2);
    }
}

unsigned int decimal_digits(unsigned int i) {
    static const unsigned limits[] =
            {0, 10, 100, 1000, 10000, 100000,
             1000000, 10000000, 100000000, 1000000000};
        register const unsigned int z = 0;
        register unsigned int l, log2;
...
int main() {
    ...
    while ((bytes_read = read(0, &buffer[8], sizeof(buffer) - 8)) > 0) {
        ...
        while (i < bytes_read) {
            ...
            if (__builtin_expect(i != bytes_read, 1)) { // Lo hemos conseguido parsear
                __builtin_prefetch(&buffer[written], 1, 3);
                // n es válido
