...
char to_string[MAX_SOL + 1][8] __attribute__ ((aligned (8)));
...
int main() {
    ...
    for (i = 0; i <= MAX_SOL; ++i) {
        ...
        unsigned int digits = decimal_digits(value);
        to_string[i][0] = digits + 1;

        char* ini = &to_string[i][digits];
        char* fi = &to_string[i][1];
    ...
    while ((bytes_read = read(0, &buffer[8], sizeof(buffer) - 8)) > 0) {
        ...
        while (i < bytes_read) {
            ...
            if (i != bytes_read) { // Lo hemos conseguido parsear
                ...
                memcpy(&buffer[written], &to_string[sum[n]][1], to_string[sum[n]][0]);
                written += to_string[sum[n]][0];
