...
unsigned char digits[MAX_SOL+1];
...
int main() {
    ...
    for (i = 10; i < 100; ++i) digits[i] = 2;
    for (i = 100; i < 1000; ++i) digits[i] = 3;
    for (i = 1000; i < 10000; ++i) digits[i] = 4;
    for (i = 10000; i < 100000; ++i)digits[i] = 5;
    for (i = 100000; i <= MAX_SOL; ++i) digits[i] = 6;
    ...
    while ((bytes_read = read(0, &buffer[8], sizeof(buffer) - 8)) > 0) {
        ...
        while (i < bytes_read) {
            ...
            if (i != bytes_read) { // Lo hemos conseguido parsear
                ...
                unsigned int n_digits = digits[value];
